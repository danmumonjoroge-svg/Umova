...
const FIELD_CLASS =
  "w-full h-13 px-4 rounded-2xl border border-slate-300 focus:border-emerald-700 focus:ring-4 focus:ring-emerald-100 outline-none text-slate-800 placeholder-slate-400 transition";
...
export default function POSTenantSignup({ onBackToLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!result.ok) {
        setError(REASON_MESSAGES[result.reason] || REASON_MESSAGES.UNKNOWN);
        return;
      }
      setSubmittedCode(result.businessCode);
    } catch (err) {
      console.error("[POS SIGNUP]", err);
      setError(REASON_MESSAGES.UNKNOWN);
    } finally {
      setLoading(false);
    }
  };
      ...
      <Shell>
        <div className="text-center py-4">
          ...
          <button onClick={onBackToLogin}
            className="w-full h-13 rounded-2xl bg-emerald-800 hover:bg-emerald-900 text-white font-medium">
            Back to Sign In
          </button>
        </div>
      </Shell>
    ...
    <Shell>
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <button type="submit" disabled={loading}
          className="w-full h-13 rounded-2xl bg-emerald-800 hover:bg-emerald-900 disabled:opacity-60 text-white font-medium flex items-center justify-center gap-2 mt-2">
    ...
    <div className="min-h-screen bg-slate-100 flex items-center justify-center px-4 py-10">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-xl border border-slate-200 p-8">
        <div className="flex items-center gap-3 mb-6">
          <div className="w-11 h-11 rounded-2xl bg-emerald-800 flex items-center justify-center flex-shrink-0">
